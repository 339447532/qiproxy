...
public class HttpRequestHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        // 错误码规则：除100取整为http状态码，但403xx错误返回200（让前端在success回调中处理）
        int httpStatus = responseInfo.getCode() / 100;
        if (httpStatus == 403) {
            httpStatus = 200;
        }
        outputContent(ctx, request, httpStatus, JsonUtil.object2json(responseInfo),
