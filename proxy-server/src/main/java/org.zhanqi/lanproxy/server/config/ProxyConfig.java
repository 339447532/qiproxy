package org.zhanqi.qiproxy.server.config;
...
import org.zhanqi.qiproxy.common.Config;
...
public class ProxyConfig implements Serializable {
    ...
    static {
        ...
        // 代理配置信息存放在用户根目录下
        String dataPath = System.getProperty("user.home") + "/" + ".qiproxy/";
    ...
    public Set<Integer> getAllocatedPorts() {
        return inetPortLanInfoMapping.keySet();
    }
     ...
     *
     * @param port 端口号
     * @return true表示已占用，false表示可用
     */
    public boolean isPortAllocated(Integer port) {
        return inetPortLanInfoMapping.containsKey(port);
    }

    /**
     * 自动分配一个可用端口，从24000开始
     *
     * @return 可用的端口号，如果找不到可用端口返回null
     */
    public Integer allocatePort() {
        for (int port = 24000; port <= 65535; port++) {
    ...
    private boolean isPortAvailable(int port) {
        if (!canBind("0.0.0.0", port)) {
            return false;
        }
        Boolean v6 = canBindIpv6(port);
        return v6 == null ? true : v6.booleanValue();
    }

    private boolean canBind(String host, int port) {
    ...
    private Boolean canBindIpv6(int port) {
        ServerSocket socket = null;
        try {
            socket = new ServerSocket();
            socket.setReuseAddress(false);
            ...
            if (msg != null && (msg.contains("Protocol family") || msg.contains("Address family") || msg.contains("not supported"))) {
                return null;
            }
